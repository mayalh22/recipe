...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const showRecipeDetails = (recipe) => {
    //show all paragraphs in a recipe card//
    recipe.querySelectorAll('p').forEach(p => {
      //show all paragraphs//
  ...
  const selectRecipe = (recipe) => {
    ...
    if (selectedRecipe) {
      //if a recipe is already selected//
    ...
    recipe.scrollIntoView({
      //scroll selected recipe into view//
      behavior: 'smooth',
      //smooth scroll//
      block: 'center',
      //center vertically//
  ...
  recipes.forEach(recipe => {
    ...
    recipe.addEventListener('click', (e) => {
      ...
      if (recipe === selectedRecipe) {
        //if clicked recipe is already selected//
        deselectRecipe();
        //deselect it//
      } else {
        //select clicked recipe//
        selectRecipe(recipe);
        //select clicked recipe//
      }
    });
  ...
  if (searchBox) {
    ...
    const performSearch = (searchTerm) => {
      //perform search and filter recipes//
  ...
  if (randomBtn) {
    //if random button exists//
    randomBtn.addEventListener('click', () => {
      //on button click//
      if (recipes.length === 0) return;
      //if no recipes, do nothing//
